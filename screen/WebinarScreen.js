...
export default function WebinarScreen(props){
    ...
    let fetchWebinar = async()=>{
        setDataLoaded(false);
        let request = await fetch(`${endpoint}/getwebinar`);
        let response = await request.json();
        console.log(response);
        setWebinar(response.map((el)=>{
            return {
                ...el,
                visible:true
            }
            ...
            </View>
            {
                ...
                <View style={{marginVertical:EStyleSheet.value("20rem"),paddingHorizontal:EStyleSheet.value("20rem")}}>
                    <View style={{backgroundColor:"white",borderWidth:1.5,borderColor:"#e8e8e8",flexDirection:"row",paddingVertical:EStyleSheet.value("5rem"),borderRadius:EStyleSheet.value("30rem")}}>
                            ...
                            timer.current = setTimeout(() => {
                                setWebinar((prev)=>{
                                    let filtered = prev.map((el)=>{
                                        ...
                                        if(el.nama_webinar.match(regex)){
                                            return {
                                                ...el,
                                                visible:true
                                            }
                        ...
                        value={query}
                        placeholder="Cari Webinar..." style={{height:EStyleSheet.value("35rem"),color:"grey",flex:1}}></TextInput>
                           <TouchableOpacity 
                            onPress={()=>{
                            setQuery("");
                            setWebinar((prev)=>{
                    ...
                    if(item.visible){
                        return (
                            ...
                            onPress={()=>{
                                ...
                                //props.navigation.navigate("DetailWebinar", {item:item});
                                Linking.openURL(item.link_youtube);
                            }}
                            >
                            <LinearGradient 
                            ...
                            style={{...shadow,backgroundColor:"#24b596",overflow:"hidden",marginBottom:EStyleSheet.value("15rem"),borderRadius:EStyleSheet.value("5rem"),display:"flex",flexDirection:"row"}}>
                                ...
                                <View style={{flex:1,justifyContent:"center"}}>
                                    ...
                                    <Text numberOfLines={1} style={{fontWeight:"bold",color:"white"}}>{item.nama_webinar}</Text>
                                </View>

...
export default function ArtikelScreen(props){
    ...
    let fetchArtikel = async()=>{
        setDataLoaded(false);
        let request = await fetch(`${endpoint}/artikel`);
        let response = await request.json();
        console.log(response);
        setArtikel(response.map((el)=>{
            return {
                ...el,
                visible:true
            }
            ...
            </View>
            {
                ...
                <View style={{marginVertical:EStyleSheet.value("20rem"),paddingHorizontal:EStyleSheet.value("20rem")}}>
                    <View style={{backgroundColor:"white",borderWidth:1.5,borderColor:"#e8e8e8",flexDirection:"row",paddingVertical:EStyleSheet.value("5rem"),borderRadius:EStyleSheet.value("30rem")}}>
                            ...
                            timer.current = setTimeout(() => {
                                setArtikel((prev)=>{
                                    let filtered = prev.map((el)=>{
                                        ...
                                        if(el.judul_artikel.match(regex)){
                                            return {
                                                ...el,
                                                visible:true
                                            }
                        ...
                        value={query}
                        placeholder="Cari Artikel..." style={{height:EStyleSheet.value("35rem"),color:"grey",flex:1}}></TextInput>
                           <TouchableOpacity 
                            onPress={()=>{
                            setQuery("");
                            setArtikel((prev)=>{
                ...
                renderItem={({item,index})=>{
                    if(item.visible){
                        return (
                            ...
                            // </Pressable>
                           <Pressable onPress={()=>{
                               props.navigation.navigate("DetailArtikel", {item:item})
                           }}>
                                ...
                                <View style={{width:EStyleSheet.value("100rem"),overflow:"hidden",backgroundColor:"whitesmoke",height:EStyleSheet.value("110rem")}}>
                                    <Image style={{height:"100%",width:"100%",overflow:"hidden"}} source={{uri:`${endpoint.replace("/api","")}/storage/public/artikel/${item.gambar_artikel}`}}></Image>
                                </View>
                                <View style={{flex:1,paddingHorizontal:EStyleSheet.value("20rem"),paddingVertical:EStyleSheet.value("15rem"),height:EStyleSheet.value("110rem")}}>
                                    <Text numberOfLines={2} style={{fontWeight:"bold",lineHeight:20,fontSize:EStyleSheet.value("14rem")}}>{item.judul_artikel}</Text>
                                    <Text numberOfLines={3} style={{marginTop:EStyleSheet.value("5rem"),lineHeight:15,color:"grey",fontSize:EStyleSheet.value("10rem")}}>{item.konten.replace(/<[^>]*>/g,"")}</Text>
                                </View>
                            </View>
                           </Pressable>
                          )

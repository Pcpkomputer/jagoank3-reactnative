...
import { StyleSheet, Text, View, Dimensions, ScrollView, FlatList, Image, Pressable, ActivityIndicator, Linking, TextInput } from 'react-native';
...
export default function EbookScreen(props){
    ...
    let fetchEbook = async()=>{
        setDataLoaded(false);
        let request = await fetch(`${endpoint}/getebook`);
        let response = await request.json();
        console.log(response);
        setEbook(response.map((el)=>{
            return {
                ...el,
                visible:true
            }
        ...
        <View style={{flex:1,backgroundColor:"white"}}>
            ...
            <View style={{...shadow,backgroundColor:"white",flexDirection:"row",paddingHorizontal:EStyleSheet.value("20rem"),alignItems:"center",height:EStyleSheet.value("55rem")}}>
                ...
                <View style={{position:"absolute",justifyContent:"center",alignItems:"center",width:Dimensions.get("screen").width}}>
                    <Text style={{fontWeight:"bold",color:"rgb(38, 180, 149)"}}>E-Book</Text>
                </View>
            </View>
            {
                (dataLoaded) &&
                ...
                <View style={{marginVertical:EStyleSheet.value("20rem"),paddingHorizontal:EStyleSheet.value("20rem")}}>
                    <View style={{backgroundColor:"white",borderWidth:1.5,borderColor:"#e8e8e8",flexDirection:"row",paddingVertical:EStyleSheet.value("5rem"),borderRadius:EStyleSheet.value("30rem")}}>
                        ...
                        value={query}
                        placeholder="Cari E-Book..." style={{height:EStyleSheet.value("35rem"),color:"grey",flex:1}}></TextInput>
                          <TouchableOpacity 
                            onPress={()=>{
                            setQuery("");
                            setEbook((prev)=>{
                    ...
                    if(item.visible){
                        return (
                            ...
                            onPress={()=>{
                                ...
                                Linking.openURL(`${endpoint}/storage/public/ebook/${item.file_ebook}`)
                            }}
                            >
                            <LinearGradient 
                            ...
                            style={{...shadow,backgroundColor:"#24b596",overflow:"hidden",marginBottom:EStyleSheet.value("15rem"),borderRadius:EStyleSheet.value("5rem"),display:"flex",flexDirection:"row"}}>
                                ...
                                <View style={{flex:1,justifyContent:"center"}}>
                                    ...
                                    <Text numberOfLines={1} style={{fontWeight:"bold",color:"white"}}>{item.nama_ebook}</Text>
                                </View>
